...
| 円満ポイント加算 | スタンプを送る・受け取ることでポイント付与 |
...
| `make e2e` | Playwright E2E テスト実行（全 55 ケース） |
...
Playwright を使用した E2E テストが 8 シナリオ・**55 テストケース**実装済みで、全て PASS しています。
...
| `02-tasks.spec.ts` | タスク作成・完了・テンプレート・バリデーション | 9 |
| `03-notifications.spec.ts` | 通知一覧・スタンプ絵文字・ナビゲーション | 4 |
| `04-dashboard.spec.ts` | グラフ・タイムライン・目標追加・レイアウト | 8 |
| `05-profile.spec.ts` | プロフィール表示・編集・ポイント・招待コード | 7 |
| `06-navigation.spec.ts` | BottomNav・Sidebar・全ページ遷移 | 13 |
| `07-family-setup.spec.ts` | グループ作成・参加・招待コード検証 | 4 |
| `08-appreciation.spec.ts` | ママ→パパ・パパ→ママ スタンプ送信 | 3 |
